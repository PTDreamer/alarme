...
//PUT data/pausemode.php
//{"id": "x", "pause" : "true"};
//PUT data/deletemode.php
//{"id": "x"};
(function($) {
    $.fn.goTo = function() {
        $('html, body').animate({
...
function loadMode(id, mode) {
    ...
    modeItem.find(".startTime").timeEntry({ show24Hours: true });
    modeItem.find(".startTime").timeEntry('setTime', mode.startTime);
    modeItem.find(".endTime").timeEntry({ show24Hours: true });
    modeItem.find(".endTime").timeEntry('setTime', mode.endTime);
...
function htmlToMode(id) {
    ...
    var sensors = modeItem.find(".sensorTableBody").find(":checked").closest("tr").each(function(index) {
        mode.disabledSensors.push(parseInt($(this).attr("sensorid")));
    });
...
function loadModeSensors(id, disabledSensors) {
    ...
    for (var i = 0; i < allSensors.devices.length; i++) {
        tr = $("<tr sensorid=" + allSensors.devices[i].id + ">");
        tr.append("<td>" + allSensors.devices[i].name + "</td>");
        console.log("append:" + allSensors.devices[i].name);
        if ((disabledSensors.indexOf(allSensors.devices[i].id)) != -1) {
        ...
        $.ajax({
            ...
            success: function(data, textStatus, jQxhr) {
                if (data.status == "success") {
	                ...
	                if(op) 
	                	setModeItemMode(this.id, "paused");
	                else
	                	setModeItemMode(this.id, "readOnly");
                } else {
                    showMessage("error", "Ooops something went wrong while saving");
...
$('.autoEnd').click(function(e) {
    handleShowHide($(this));
});

function handleShowHide(element) {
    if (element.prop("checked") == true) {
        element.closest(".mainBody").find(".panel-body").first().show();
    } else {
        element.closest(".mainBody").find(".panel-body").first().hide();
    }
};
